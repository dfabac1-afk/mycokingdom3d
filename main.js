...
class Game3D {
    ...
    setupStartScreen() {
        ...
        const titleSz = isMobile ? 28 : 48;
        const subSz   = isMobile ? 9 : 13;
        const lvlSz   = isMobile ? 9 : 10;
        const padIn   = isMobile ? 18 : 50;
        const gapSz   = isMobile ? 8 : 15;
        const startFs = isMobile ? 14 : 18;
        const btnFs   = isMobile ? 10 : 12;
        const smFs    = isMobile ? 9 : 10;
        const topPad  = isMobile ? 'calc(8px + env(safe-area-inset-top))' : '40px';
        const botPad  = isMobile ? 'calc(12px + env(safe-area-inset-bottom))' : '40px';
        ...
        const compactWalletHint = access.walletVerified
            ? walletHint
            : isMobile
        ...
        const cloudStatusBody = access.walletVerified || this.walletSessionToken
            ? cloudStatus.body
            : 'Guest progress saves locally on this device until you connect Phantom.';
        const holderPerksPanelId = 'holder-perks-panel';
        const holderPerksToggleId = 'holder-perks-toggle';
        const holderPerksMarkup = `
            <button id="${holderPerksToggleId}" style="width:100%; padding:${isMobile ? '11px 12px' : '10px 12px'}; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); color:#d9e6df; font-family:inherit; font-size:${isMobile ? 9 : 8}px; cursor:pointer; border-radius:12px; ${tapBtn}">
                VIEW HOLDER TIERS
            </button>
            <div id="${holderPerksPanelId}" style="display:none; width:100%; grid-template-columns:${isMobile ? '1fr' : 'repeat(2, minmax(0, 1fr))'}; gap:8px;">
        ...
        const openerHighlightsMarkup = isMobile
            ...
            `
            : `
                <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:8px; width:100%; margin-bottom:16px; position:relative; z-index:1; text-align:left;">
                    ${openerHighlights.map(item => `
                        <div style="padding:9px 12px; background:rgba(255,255,255,0.035); border:1px solid rgba(255,255,255,0.08); border-radius:999px; display:flex; align-items:center; gap:8px;">
                            <div style="font-size:8px; color:${item.accent}; letter-spacing:1px;">${item.label}</div>
                            <div style="font-size:8px; color:#d4ded8; line-height:1.45;">${item.value}</div>
                        </div>
                    `).join('')}
                </div>
            `;
                ...
                <div id="start-screen" style="display: flex; flex-direction: column; align-items: center; width: 100%; max-width: ${isMobile ? 340 : 560}px; margin: 0 auto; background: linear-gradient(180deg, rgba(10,16,18,0.96), rgba(3,7,8,0.92)); padding: ${isMobile ? 16 : 22}px ${isMobile ? 15 : 22}px ${isMobile ? 16 : 22}px; border: 1px solid rgba(255,255,255,0.1); border-top: 3px solid rgba(57,255,20,0.82); border-radius: 22px; box-shadow: 0 28px 80px rgba(0,0,0,0.52), 0 0 32px rgba(57,255,20,0.14), inset 0 1px 0 rgba(255,255,255,0.08); text-align: center; box-sizing: border-box; backdrop-filter: blur(10px); position: relative; overflow: hidden;">
                    ...
                    <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:${isMobile ? 6 : 8}px; width:100%; margin-bottom:${isMobile ? 12 : 16}px; position:relative; z-index:1;">
                        ${heroSignals.map(signal => `
                            <div style="padding:${isMobile ? '7px 9px' : '7px 10px'}; min-width:${isMobile ? '92px' : '96px'}; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:12px; text-align:left; box-shadow:inset 0 1px 0 rgba(255,255,255,0.05);">
                    ...
                    <div style="position:relative; z-index:1; display:flex; flex-direction:column; align-items:center; gap:${isMobile ? 6 : 8}px; margin-bottom:${isMobile ? 14 : 18}px;">
                        <div style="padding:6px 12px; background:rgba(57,255,20,0.1); border:1px solid rgba(57,255,20,0.24); border-radius:999px; font-size:${isMobile ? 7 : 8}px; color:#c6ffbf; letter-spacing:${isMobile ? 1.1 : 1.4}px;">PSYCHEDELIC ACTION RPG · LIVE KINGDOM BUILD</div>
                        ...
                        <p style="font-size: ${subSz}px; margin: 0; color: #f5fff5; letter-spacing: ${isMobile ? 1.1 : 2}px; line-height: 1.45; max-width: ${isMobile ? 220 : 390}px;">${subtitleHtml}</p>
                        <p style="font-size: ${lvlSz}px; margin: 0; color: #b6c8c0; letter-spacing: 1px; line-height:1.55; max-width:${isMobile ? '100%' : '430px'};">${statusLine}</p>
                    </div>
                    ...
                    <div style="display: flex; flex-direction: column; gap: 8px; align-items: center; width: 100%; position:relative; z-index:1;">
                        <div style="width:100%; padding:${isMobile ? 10 : 11}px; background:rgba(255,255,255,0.035); border:1px solid rgba(255,255,255,0.08); border-radius:16px; box-sizing:border-box; display:grid; gap:8px; text-align:left;">
                            ...
                            <div style="display:grid; grid-template-columns:${isMobile ? '1fr' : 'minmax(0, 1.15fr) minmax(0, 0.85fr)'}; gap:8px; align-items:center;">
                                <button id="wallet-button" style="width: 100%; padding: 11px 14px; font-size: ${smFs}px; background: ${access.walletVerified ? 'rgba(0,255,255,0.14)' : isConnected ? '#333' : 'rgba(106,13,173,0.16)'}; border: 1px solid ${access.walletVerified ? '#00ffff' : '#6a0dad'}; color: white; font-family: inherit; cursor: pointer; ${tapBtn}">
                                    ${access.walletVerified ? 'SYNC WALLET + CLOUD SAVE' : walletLabel}
                                </button>
                                <div style="font-size: 7px; color: #a0aba6; line-height: 1.5; max-width: 100%;">${isMobile ? mobileWalletHint : compactWalletHint}</div>
                            </div>
                            ${showCloudStatusCard ? `
                                <div style="width: 100%; padding: 8px 10px; background: rgba(0,0,0,0.28); border: 1px solid rgba(0,255,255,0.2); border-radius: 10px; text-align: left; box-sizing: border-box;">
                                    <div style="font-size: 8px; color: #00ffff; letter-spacing: 1px; margin-bottom: 4px;">${cloudStatus.title}</div>
                                    <div style="font-size: 7px; color: #8fa6a2; line-height: 1.5;">${cloudStatusBody}</div>
                                </div>
                            ` : ''}
                        </div>
                        ${holderPerksMarkup}
                    ...
                    <div style="margin-top: ${isMobile ? 8 : 14}px; width: 100%; display: flex; flex-direction: ${isMobile ? 'column' : 'row'}; align-items: ${isMobile ? 'stretch' : 'center'}; justify-content: space-between; gap: ${isMobile ? 6 : 10}px; padding: ${isMobile ? '8px 10px' : '10px 12px'}; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; position:relative; z-index:1;">
                        <div style="text-align: left; flex: 1;">
                            <div style="font-size: ${isMobile ? 8 : 9}px; color: #ccc; letter-spacing: 1px;">GRAPHICS PROFILE</div>
                            <div style="font-size: ${isMobile ? 6 : 7}px; color: #8b9992; margin-top: 3px; line-height: 1.45;">
                                ${graphicsHelper}
                            </div>
                        </div>
                        <button id="low-perf-toggle" style="padding: ${isMobile ? '8px 12px' : '9px 12px'}; font-size: ${isMobile ? 8 : 9}px; background: ${this.progression.data.settings.lowPerfMode === true ? '#ff8800' : this.progression.data.settings.lowPerfMode === false ? '#39FF14' : '#444'}; color: ${this.progression.data.settings.lowPerfMode === false ? '#000' : '#fff'}; border: none; font-family: inherit; cursor: pointer; min-width: ${isMobile ? '0' : '84px'}; width: ${isMobile ? '100%' : 'auto'}; ${tapBtn}">
                            ${this.progression.data.settings.lowPerfMode === true ? 'BATTERY' : this.progression.data.settings.lowPerfMode === false ? 'HIGH' : 'AUTO'}
                        </button>
                    </div>

                    <div style="margin-top: ${isMobile ? 10 : 20}px; font-size: ${isMobile ? 8 : 8}px; color: #7e8b85; letter-spacing: 1px; opacity: 0.95; position:relative; z-index:1;">
            ...
            <div class="mode-card" id="mode-${id}"
                ...
                <div style="position:relative; z-index:1; display:flex; justify-content:space-between; gap:12px; align-items:flex-start; margin-bottom:${isMobile ? 10 : 14}px;">
                    <div>
                        ...
                        <p style="color:#98a39e; font-size:${isMobile ? 8 : 9}px; letter-spacing:1.1px; margin:0; line-height:1.45;">${subtitle}</p>
                    </div>
                ...
                <ul style="position:relative; z-index:1; color:#dfe7e2; font-size:${isMobile ? 9 : 11}px; line-height:${isMobile ? 1.55 : 1.7}; padding-left:${isMobile ? 16 : 18}px; margin:0 0 ${isMobile ? 12 : 18}px 0; display:grid; gap:${isMobile ? 4 : 6}px;">
                    ${notes.slice(0, isMobile ? 2 : notes.length).map(note => `<li style="color:${note.accent || '#dfe7e2'};">${note.text}</li>`).join('')}
                </ul>
                ...
                <button id="pick-${id}" style="position:relative; z-index:1; width:100%; padding:${isMobile ? 11 : 13}px; background:linear-gradient(135deg, ${accent}, ${accent === '#39FF14' ? '#7dff64' : accent === '#ff8a3d' ? '#ffb36d' : '#d08dff'}); border:none; color:${buttonTextColor}; font-family:inherit; font-weight:bold; cursor:pointer; box-shadow:0 14px 30px rgba(0,0,0,0.24);">
                    ${buttonLabel}
                </button>
            </div>
        `;
        this.uiOverlay.innerHTML = `
            <div style="pointer-events: auto; display: flex; flex-direction: column; align-items: center; width: 100%; min-height: 100%; background: radial-gradient(circle at top, rgba(57,255,20,0.06), transparent 26%), linear-gradient(180deg, rgba(0,0,0,0.92), rgba(2,5,6,0.96)); padding:${isMobile ? '18px 14px 24px' : '30px 24px 36px'}; overflow-y: auto; box-sizing:border-box;">
                <div style="width:100%; max-width:1180px; display:grid; gap:${isMobile ? 18 : 24}px;">
                    <div style="background:linear-gradient(180deg, rgba(9,14,16,0.95), rgba(4,7,8,0.92)); border:1px solid rgba(255,255,255,0.08); border-top:3px solid #39FF14; border-radius:22px; padding:${isMobile ? 16 : 24}px; box-shadow:0 24px 64px rgba(0,0,0,0.42); position:relative; overflow:hidden;">
                        ...
                        <div style="position:relative; z-index:1; display:grid; gap:${isMobile ? 12 : 14}px;">
                            <div style="display:flex; flex-wrap:wrap; gap:${isMobile ? 6 : 8}px; align-items:center;">
                                ...
                                <div style="padding:7px 12px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.08); border-radius:999px; font-size:${isMobile ? 7 : 8}px; color:#b7c0bc; letter-spacing:1.2px;">CURRENT: ${this.getGameModeLabel()}</div>
                                <div style="padding:7px 12px; background:${territoryUnlocked ? 'rgba(255,138,61,0.12)' : 'rgba(255,255,255,0.04)'}; border:1px solid ${territoryUnlocked ? 'rgba(255,138,61,0.24)' : 'rgba(255,255,255,0.08)'}; border-radius:999px; font-size:${isMobile ? 7 : 8}px; color:${territoryUnlocked ? '#ffb36d' : '#b7c0bc'}; letter-spacing:1.2px;">${territoryUnlocked ? 'LIVE WAR UNLOCKED' : `${access.minimum.toLocaleString('en-US')} MYCO FOR WAR`}</div>
                            </div>
                            <div>
